...
export class DocumentDownloader {
  ...
  private async downloadSingleDocument(filename?: string): Promise<string> {
    ...
    const formData = await this.page.evaluate(() => {
      ...
      // Parse: goDLS('/doc1/121112427138','475036','30','1','','1','','','')
    ...
    const fullUrl = formData.action.startsWith('http') 
      ? formData.action 
      : `https://ecf.nmd.uscourts.gov${formData.action}`;

    // Make POST request with form data
    // Note: Setting Referer to bypass CSRF warning page
    const response = await axios({
      ...
      headers: {
        ...
        'Referer': 'https://ecf.nmd.uscourts.gov/',
